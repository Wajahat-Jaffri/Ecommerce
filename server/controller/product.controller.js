...
// Get all products (With Serverless Buffering & Empty States handling fixed)
export const fetchAllProducts = async (req, res) => {
  try {
    // Check if database connection is fully active before querying
    if (mongoose.connection.readyState !== 1) {
      return res.status(200).json({
        success: true,
        message: "Database connecting... returning empty backup",
        data: []
      });
    }

    // .lean() query performance aur timeouts se bachne ke liye zaroori hai
    const listOfProducts = await Product.find({}).lean().exec();
    
    return res.status(200).json({
      success: true,
      message: "Products fetched successfully",
      data: listOfProducts || [] // Agar collection khali bhi ho to crash na kare, khali array jaye
    });
  } catch (error) {
    console.error("Product fetch failed:", error);
    // Crash se bachane ke liye safe fallback list bhejein taake front-end white screen na ho
