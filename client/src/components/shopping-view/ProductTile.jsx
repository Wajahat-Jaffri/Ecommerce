...
function ShoppingProductTile({ 
  ...
  return (
    <Card className="group border border-gray-100 bg-white/50 backdrop-blur-sm rounded-[1.5rem] overflow-hidden transition-all duration-500 w-full max-w-[280px] mx-auto">
      ...
      {/* Upper Area: Clickable for Product Details */}
      <div 
        onClick={() => handleGetProductDetails(product?._id)} 
      ...
      {/* Footer Area: Add to Cart Button */}
      <CardFooter className="p-4 pt-0">
        {product?.totalStock === 0 ? (
          <Button 
            disabled 
            className="w-full bg-gray-100 text-gray-400 rounded-xl h-10 uppercase text-[9px] font-black tracking-widest cursor-not-allowed"
          >
            Unavailable
          </Button>
        ) : (
          <Button 
            onClick={(e) => {
              e.stopPropagation(); // Card کے کلک ایونٹ کو روکنے کے لیے
              handleAddtoCart(product?._id, product?.totalStock);
            }}
            className="w-full bg-[#1f2937] hover:bg-[#be185d] text-white rounded-xl h-10 uppercase text-[9px] font-black tracking-widest transition-all duration-300 group/btn shadow-md shadow-gray-100 hover:shadow-pink-100"
          >
            <ShoppingCart className="w-3 h-3 mr-1.5 transition-transform group-hover/btn:-rotate-12" />
            Add to Selection
          </Button>
